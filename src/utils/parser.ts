...
export function normalizePlaceDetails(formValues: PlaceFormValues): PlaceDetails {
  ...
  const coordinates: [number, number] = [
    parseFloat((formValues?.longitude || "0").trim()) || 0,
    parseFloat((formValues?.latitude || "0").trim()) || 0,
  ];

  const startMonth = formValues?.bestSeason?.startMonth || "October";
  const endMonth = formValues?.bestSeason?.endMonth || "March";
  const bestSeasonStr = `${startMonth} to ${endMonth}`;

  const openingHoursStr = formValues?.openingHours
    ? JSON.stringify(formValues.openingHours)
    : JSON.stringify({
        mode: "24h",
        sameTime: { start: "09:00 AM", end: "06:00 PM" },
        days: {
  ...
  return {
    ...
    transportType: (formValues?.transportType || "Bus").trim(),
    coordinates,
